...
Whipple&\{John Adams Whipple, John A. Whipple\} \\ \hline
\hline\end{tabular}
\end{table*}


In the rule-based method we block based on items that have one word in common. This is the simplest reasonable blocking strategy. In table 2 we see a what the blocking of the three names from table 1 would look like. Out of NUMBER1 pairs NUMBER2 ended up in the same bucket as all their correct matches. NUMBER3 had at least one correct match in the same bucket. To preform the actuall matching we used three simple rules. One, if two items share a word that only those two have in common match them. Two, if the we remove spaces and one of the items is a substring of the other match them. Three, if we treat each word as a member of a set representing the entity, and one set is a subset of the other we match them. We found that most of the problem with this method was not enough positives, so we accepted if any of the rules matched the two items. If we treat it as a success if the correct match is any of the top 3 we get an fscore of FSCORE1. Since the issue is mostly false-negitives, even if we accept any of the top 1000 as a success, we get FSCORE2, not much better.

The first part we replaced was the matcher. To compare the two matchers we created a rule-based matcher using the rules from the rule-based system. To replace it we created a siamese network (figure 1) INSERT FIGURE 1. A Siamese network is two deep neural networks that share weights. One entity is fed into each of the two networks. The output of the two networks is fed into a final layer which determines the distance between the two outputs and accepts or rejects. Siamese networks have been shown to work well for image matching.\footnote{CITE} Before feeding the entities into the networks, we used Kazuma charecter embeddings to encode each entity as a vector. We trained the network on our NUMBER1*0.95 pairs and withheld NUMBER1*0.05 for testing. We chose an equal number of negitive pairs to train the model on. We chose negitive pairs that had at least one word in common with the positive ones. We found that the fscore on the training data was FSCORE3 and FSCORE4 on the test date. This was better than the rule-based matcher which had an f-score of FSCORE5.
...
There have been a number of attemts to solve the fuzzy join problem. Many of the attemts use varios string matching algorithms for example [Fast-Join: An Efficient Method for Fuzzy TokenMatching based String Similarity Join]. The main issue with using string matching comparisons is that they do not work across a wide variaty of enties ASK DR. SRINIVAS. To our knowledge no serios machine learning aproach has been used for this problem. The problem of blocking has also been dealt with using MapReduce [Efficient Parallel Set-Similarity Joins Using MapReduce]. This approach requires alot of computing power and, since they use a string matching approach in the end, is hard to generalize accross many types of entities. 
